...
Route::get('/', function () {
    if (auth()->check()) {
        return redirect()->route('pemesanan'); // Jika sudah login, arahkan ke halaman pemesanan
    }
    return view('welcome'); // Jika belum login, tampilkan halaman welcome
});
Route::get('/Pemesanan', [PemesananController::class, 'index'])->name('pemesanan')->middleware('auth', 'verified');
...
Route::prefix('admin')
    ...
    ->group(function () {
        ...
        Route::get('pdf', [TransactionController::class, 'generatePDF'])->name('admin-transaction.pdf');
...
Route::middleware('auth', 'checkrole:pemilik', 'verified')->group(function () {
...
Route::middleware('auth', 'checkrole:pencari', 'verified')->group(function () {
